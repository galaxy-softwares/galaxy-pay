...
export class RefundTradeService extends BaseService<RefundTrade> {
  ...
  async find(): Promise<any> {
    try {
      const income = await this.refundTradeRepository
        ...
        .getRawOne();
      const data = await this.refundTradeRepository
        .createQueryBuilder('trade')
   ...
   */
  async refundTradeSuccess(
    out_trade_no: string,
    channel: TradeChannel,
    trade_no: string,
  ): Promise<RefundTrade> {
    try {
      const refund_trade = await this.findUnPaidTrade(out_trade_no, channel);
      if (refund_trade) {
